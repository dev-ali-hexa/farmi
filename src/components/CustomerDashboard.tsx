...
import { 
  ...
  Map, Trash2, ArrowRight, Check, Heart, HelpCircle, RefreshCw, Sparkles, ChevronRight, Download, Ticket, QrCode
} from 'lucide-react';
    ...
    printWindow.document.write(`
      <html>
        <head>
          ...
          <style>
            body { font-family: monospace; padding: 40px; color: #1c1917; }
            .header { border-bottom: 2px solid #e7e5e4; padding-bottom: 20px; margin-bottom: 20px; }
            .item { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px dashed #e7e5e4; }
            .total { font-weight: bold; font-size: 1.2em; margin-top: 20px; text-align: right; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>FURNIDESIGN - OFFICIAL INVOICE</h2>
            <p>Order ID: ${order.id?.toUpperCase() || 'UNKNOWN'}</p>
            <p>Date: ${new Date(order.createdAt).toLocaleDateString()}</p>
            <p>Customer: ${order.customerName}</p>
            <p>Delivery: ${order.shippingAddress}</p>
          </div>
          <h3>Items Purchased:</h3>
          ${(order.items || []).map(i => `<div class="item"><span>${i.name} (x${i.quantity})</span><span>Rs. ${i.price * i.quantity}</span></div>`).join('')}
          <div class="total">Total Paid Amount: Rs. ${order.totalAmount} ${order.promoCode ? `<br><small style="font-weight:normal; color:#666;">(Includes Promo: ${order.promoCode})</small>` : ''}</div>
          <p style="margin-top: 50px; font-size: 0.8em; color: #78716c; text-align: center;">Thank you for trusting FurniDesign.</p>
        </body>
      </html>
    `);
